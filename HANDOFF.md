...
- **Transient HTTPS drops.** This PC's antivirus/proxy intercepts HTTPS; under load it occasionally resets a connection (`WinError 10054`). MusicBrainz calls have a **30s timeout + 2 retries** (`musicbrainz.network_timeout` / `retries` / `retry_backoff`) so a dead connection **can't freeze the watcher** and most resets are retried through. (This was fixed 2026-06-18 after a hang.) An album that still fails every retry is left in `review` (safe). There's no dedicated "review re-sweep" mode yet — `--once --force` re-judges everything (slow); possible future `--review-resweep`.
